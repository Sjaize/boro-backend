...
| Method | Endpoint | Description | Request | Response |
| --- | --- | --- | --- | --- |
| GET | /api/users/me | 내 프로필 및 마이페이지 요약 정보 조회 (닉네임, 프로필 이미지, 동네, 신뢰도, 빌린 횟수, 빌려준 횟수, 관심목록의 개수, 주변 긴급 알림 수신 여부) | 없음 (Authorization 헤더 ****사용) | { "data": { "id": 1, "nickname": "홍길동", "profile_image_url": "https://...", "region_name": "영통동", "trust_score": 4.8, "borrow_count": 3, "lend_count": 5, "like_count": 12, "nearby_urgent_alerts_enabled": false } } |
...
| PATCH | /api/users/me/settings | 사용자 설정 변경 (알림 반경, 관심 키워드, 주변 긴급 알림 수신 여부 / 일부 필드만 전송 가능) | { "notification_radius_m": 1500, "interest_keywords": ["전자기기", "가구"], "nearby_urgent_alerts_enabled": true } | { "data": { "notification_radius_m": 1500, "interest_keywords": ["전자기기", "가구"], "nearby_urgent_alerts_enabled": true } } |
...
| POST | /api/notifications/device-tokens | 디바이스 푸시 토큰 등록 또는 갱신 | { "device_token": "fcm-device-token-value", "platform": "android" } | { "data": { "device_token": "fcm-device-token-value", "platform": "android", "is_active": true } } |
| DELETE | /api/notifications/device-tokens | 디바이스 푸시 토큰 비활성화 (로그아웃/앱 제거 시 사용) | { "device_token": "fcm-device-token-value" } | { "data": { "device_token": "fcm-device-token-value", "deleted": true } } |
| PATCH | /api/notifications/{notification_id}/read | 특정 알림 읽음 처리 | 없음 | { "data": { "id": 3, "is_read": true } } |

- `urgent_post` 푸시는 디바이스 토큰이 등록된 사용자에게만 전송 시도됩니다.
- 서버는 `nearby_urgent_alerts_enabled=true`, `location_updated_at` 최근 6시간 이내, `notification_radius_m` 반경 이내인 사용자만 긴급 알림 대상으로 판단합니다.
