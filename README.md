...
# 백그라운드에서 서비스 실행
docker compose up -d
alembic init alembic     →로컬에서 마이그레이션 설정 초기화
alembic revision --autogenerate -m "init"  → models 읽어서 migration 파일 생성
alembic upgrade head     → DB에 테이블 생성
